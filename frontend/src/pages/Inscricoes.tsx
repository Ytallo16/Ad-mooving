...
const Inscricoes = () => {
  ...
  const startPixStatusPolling = (pixId: string) => {
    ...
    // Contador de erros consecutivos — só cancela após 5 falhas seguidas
    ...
    const interval = setInterval(async () => {
      try {
        ...
        if (resp.ok && data?.success) {
          consecutiveErrors = 0; // resetar contador em sucesso
          if (data.status === 'PAID') {
            setPixStatus('paid');
          ...
          consecutiveErrors++;
          console.warn(`DEBUG PIX: Erro ao verificar status (${consecutiveErrors}/${MAX_ERRORS})`);
          if (consecutiveErrors >= MAX_ERRORS) {
            ...
            toast({
              ...
              description: "Não foi possível verificar o pagamento do PIX. Se você já pagou, seu pagamento será confirmado em breve.",
              variant: "destructive"
            });
        ...
        consecutiveErrors++;
        console.error(`Erro ao verificar status do PIX (${consecutiveErrors}/${MAX_ERRORS}):`, error);
        if (consecutiveErrors >= MAX_ERRORS) {
          clearInterval(interval);
          setPollingInterval(null);
          setShowPixModal(false);
          toast({
            title: "Erro de conexão",
            description: "Não foi possível verificar o pagamento. Se você já pagou, seu pagamento será confirmado em breve.",
            variant: "destructive"
          });
          navigate('/pagamento/cancelado', { replace: true });
        }
